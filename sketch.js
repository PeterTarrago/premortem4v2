...
const MAX_TOUCHES = 50; // Número máximo de toques antes de la "muerte" (constante en esta versión)
...
const sketch = (p) => {
    ...
    p.preload = () => {
        ...
        imagenglitchImg = p.loadImage('imagenglitch.png');
    };
    ...
    p.setup = () => {
        ...
        // Usaremos una onda cuadrada para un sonido más "digital" y áspero, estilo Ryoji Ikeda.
        osc = new p5.Oscillator('square'); // Cambiado a onda cuadrada para un sonido más "duro"
    ...
    p.touchStarted = () => {
        // Si el celular ya está "muerto", no hay interacción ni sonido.
        if (isDead) {
            return false;
        }

        // Asegura que el contexto de audio se inicie con la primera interacción del usuario
        // Y SOLO si el dispositivo NO está muerto y el audio aún no ha iniciado.
        if (!isAudioStarted) {
            p.userStartAudio();
    ...
    p.windowResized = () => {
        ...
        p.resizeCanvas(p.windowWidth, p.height); // Se mantiene la altura para evitar deformaciones
    ...
    function resetSketch() {
        ...
        counter = 0;
        // MAX_TOUCHES se mantiene como const en esta versión, no se re-randomiza
        glitches = [];
        isDead = false;
        currentMessage = "Toca la pantalla para iniciar el ciclo...";
